...
export default function SelectFestival() {
    ...
    const handleChange = () => {
        ...
        //해당 데이터로 카드 만들기
        tm= tm.map((item)=> <TailCard key={item.UC_SEQ} title={(item.MAIN_TITLE).split('(')[0]} subtitle={item.MAIN_PLACE} imgurl={item.MAIN_IMG_NORMAL} keyword={item.USAGE_DAY_WEEK_AND_TIME}/>);
        //만든 태그들 스테이트변수에 넣어주기
        setCards(tm);
    }
  return (
